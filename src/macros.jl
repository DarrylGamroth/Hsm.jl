...
- `event`: The event symbol this handler responds to
...
macro on_event(state, event, def)
    ...
    argname = length(def.args[1].args) > 1 ? def.args[1].args[2] : :(arg)
    
    return quote
        function Hsm.on_event!($smarg::$(esc(smtype)), ::$(esc(:Val)){$state}, ::$(esc(:Val)){$event}, $argname)
            $body
        end
    end
end
...
hierarchical state machine. It adds `_current` and `_source` fields and implements the
required methods for `Hsm.current`, `Hsm.current!`, `Hsm.source`, and `Hsm.source!`.

# Features
- Adds `_current` and `_source` fields automatically (initialized to `Hsm.Root`)
...
- Field names `_current` and `_source` are reserved and cannot be used
...
macro hsmdef(struct_expr)
    ...
    # Add _current and _source fields
    push!(fields, :(_current::Symbol))
    push!(fields, :(_source::Symbol))
    
    # Calculate number of user fields (without _current and _source)
    ...
    result = quote
        ...
        # Add convenience constructor that adds Root for state fields
        function $(esc(struct_name))(args::Vararg{Any, $num_user_fields})
            return $(esc(struct_name))(args..., Hsm.Root, Hsm.Root)
        end
