...
| HSM-PERF-001 | Warmed library-owned choice, history, completion, and terminate paths are type-stable and allocate zero heap bytes when callbacks do not allocate. | Project performance contract | finite value splits and generated static kernels | `test_history.jl` dynamic and AllocCheck evidence, `test_choice.jl`, `test_completion.jl`, `test_terminate.jl`, `test_type_stability.jl` | Implemented |
